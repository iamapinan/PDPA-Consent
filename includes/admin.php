...
class PDPA_Admin
{
    ...
    public function _section_fields()
    {
        ...
        */
        ...
        add_settings_field(
            'list_data', // id
            __('List of user data (Direct)<br>(one data set per line)', 'pdpa-consent'), // title
        ...
        add_settings_field(
            'list_data2', // id
            __('List of user data (none direct)<br>(one data set per line)', 'pdpa-consent'), // title
            array( $this, 'list_data2_callback' ), // callback
            'settings', // page
            '_pdpa_setting_section' // section
        );
        add_settings_field(
    ...
    public function list_data2_callback()
    {
        printf(
            '<textarea class="large-text" rows=5 name="pdpa_option[list_data2]" id="list_data2" placeholder="%s" required>%s</textarea>',
            __("IP Address\nMobile number\nEmail."),
            isset($this->options['list_data2']) ? esc_html($this->options['list_data2']) : ''
        );
    }
