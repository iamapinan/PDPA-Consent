...
class PDPA_Consent
{
    ...
    private function includes () {
        // Includes
        if (is_admin()) {
            require_once(PDPA_PATH . 'includes/admin.php');
            new PDPA_Admin;
        }

        require_once(PDPA_PATH . 'includes/template.php');
        require_once(PDPA_PATH . 'includes/user.php');
        new PDPA_User;
    }
    ...
    public function init()
    {
        ...
        add_filter('manage_users_custom_column', array( $this, 'pdpa_add_user_column_data' ), 99, 3);
    }
    ...
    {
        wp_enqueue_style($this->plugin_info['TextDomain'], plugins_url('assets/pdpa-consent.css', __FILE__), array(), $this->plugin_info['Version'], 'all' );
    ...
    public function pdpa_ajax_do()
    {
        ...
        $consent_fields = [
            ...
            'direct' => $this->clean_json( $_POST['direct_data'] ),
            'none_direct' => $this->clean_json( $_POST['none_direct_data'] ),
        ...
        $this->pdpa_mail($consent_fields);
        wp_send_json($response, 200);
        wp_die();
    }

    function pdpa_mail($mail) {
        $list_direct = implode('</li><li>', json_decode($mail['direct']));
        $list_none_direct = implode('</li><li>', json_decode($mail['none_direct']));
        $message = "
        <h3>PDPA Consent Recipe</h3>
        <p>Dear, Data Owner.<br>
        This is automatice generated email from PDPA consent.<br>
        We're attached recipe file for your consent below.<br>
        You don't need to reply this email.</p>
        <p><b>Latest status: <b> ".$mail['status']."</p>
        <p><b>Your consent id: <b> 973481374</p>
        <p><b>Your consent data list.</b></p>
        ...
        ---<br>
        <p>Best regards / ".$mail['timestamp']."</p>";
        $subject = "PDPA Consent Recipe.";

        if($mail['user_mail'] != '')
            wp_mail($mail['user_mail'], $subject, $message);
    }
...
if ( function_exists( 'is_multisite' ) && is_multisite() ) {
	add_action('plugins_loaded', 'pdpa_load' );
} else {
	pdpa_load();
}